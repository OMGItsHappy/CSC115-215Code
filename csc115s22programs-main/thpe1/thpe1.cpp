...
string getCCType(string cardNumber) 
    ...
    { 
        for (int i = 0; i < 6; i++) 
        {
            ...
            if (intOfNum(cardNumber, 2) == 65 || intOfNum(cardNumber, 4) == 6011
            || (644 <= intOfNum(cardNumber, 3) && 649 >= intOfNum(cardNumber, 3)) 
            || (622126 <= intOfNum(cardNumber, 6) && 622926 >= intOfNum(cardNumber, 6))
            && cardNumber.size() == 16) 
            {
                return "Discover";
            }
